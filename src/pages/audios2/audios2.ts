...
export class Audios2Page {
  ...
  getAlbum() {
    ...
    this.service.enviaContador('albuns', this.itemAlbum['idalbum']).subscribe();
  ...
  executeNextAudio() {
    ...
    setTimeout(() => {
      ...
      this.audioService.audio = 'http://app.progettoapp.com.br/arquivos/r/albuns/' + this.relAudios[this.audioService.indexAudio].arquivo_audio;
  ...
  tooglePlay() {
    ...
    if (!this.audioService.audio) {
      if (this.relAudios.length > 0) {
        this.audioService.audio = 'http://app.progettoapp.com.br/arquivos/r/albuns/' + this.relAudios[0].arquivo_audio;
        this.audioService.audioPlayer.nativeElement.src = this.audioService.audio;
  ...
  startAudio(item, index) {
    ...
    this.service.enviaContador('audios', item.idaudio).subscribe();
...
*/
  ...
  AddMinhaListaAudio(item, index, itemAlbum) {
    let indexToRemove = 0;
    ...
    this.Storage.get("MinhaListaAudios").then((data) => {
      ...
      if (filteredAudio.length > 0) {
        ...
        data.push({
          ...
          "Ativo": item.Ativo,
          "idalbum": item.idalbum,          
          "Titulo": itemAlbum.Titulo,
          "Subitulo": itemAlbum.Subitulo,
          "Descricao": itemAlbum.Descricao,
          "Pasta": itemAlbum.Pasta,
          "Img": itemAlbum.Img,
          "Caminho": itemAlbum.Caminho,
          "Valor": itemAlbum.Valor,
          "Desconto": itemAlbum.Desconto,        
        });
  ...
  getCurtirIcon(idAudio) {
    ...
    if (filteredAudio.length > 0) {
      return 'ico-curtir-podcast';
    } else {
      return 'ico-curtir-podcast-off';
    }
  }
  ...
  ClassficarAudio(item) {
    ...
    this.Storage.get("ClassficarAudios").then((data) => {
      if (data == null || data.length == 0) {
        ...
        /*this.toastCtrl.create({
          message: "Você marcou o gostei nesse podcast!",
          duration: 2000,
          position: 'middle',
        }).present();*/
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          if (item.idaudio == data[i].idaudio) {
            ...
            /*this.toastCtrl.create({
              message: "Você desmarcou o gostei nesse podcast!",
              duration: 2000,
              position: 'middle',
            }).present();*/
